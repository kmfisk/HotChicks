...
public class ChickenBreedGoal extends Goal {
    ...
    public void tick() {
        ...
        ++this.spawnBabyDelay;
        if (this.spawnBabyDelay >= 60 && this.animal.getDistanceSq(this.targetMate) < 9.0D && this.targetMate.getSex() == Sex.MALE && this.animal.getSex() == Sex.FEMALE) {
            this.spawnBaby();
        }
    ...
    private HotChickenEntity getNearbyMate() {
        List<HotChickenEntity> list = this.world.getTargettableEntitiesWithinAABB(this.mateClass, field_220689_d, this.animal, this.animal.getBoundingBox().grow(8.0D));
