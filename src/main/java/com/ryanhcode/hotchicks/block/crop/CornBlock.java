...
public class CornBlock extends BushBlock implements IGrowable {
    ...
    public CornBlock(Properties properties, RegistryObject<Item> item) {
        super(properties);
        this.registerDefaultState(this.getStateDefinition().any().setValue(AGE, 0).setValue(TYPE, 0));
        this.item = item;
    }
    ...
    private void growTo(BlockState state, ServerWorld world, BlockPos pos, boolean boned) {
        ...
        if (newAge == 2) {
            ...
            setAge(world, pos, newAge);
            //setAge(world, pos.above(), newAge);
            //setAge(world, pos.above().above(), newAge);
        }
    }
    ...
    public boolean isBonemealSuccess(World worldIn, Random rand, BlockPos pos, BlockState state) {
        return true;
        //return isValidToGrow(worldIn.getBlockState(pos.above())) && isValidToGrow(worldIn.getBlockState(pos.above().above()));
    }
