...
import com.ryanhcode.hotchicks.block.TroughFillType;
...
public class PepperBerryBush extends BushBlock implements IGrowable {
    ...
    public PepperBerryBush(Properties properties, RegistryObject<Item> item) {
        super(properties);
        this.setDefaultState(this.stateContainer.getBaseState().with(AGE, Integer.valueOf(0)).with(VARIANT, PepperType.getRandom()));
        this.item = item;
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        if (!flag && player.getHeldItem(handIn).getItem() == Items.BONE_MEAL) {
            ...
            worldIn.setBlockState(pos, state.with(AGE, Integer.valueOf(2)).with(VARIANT, PepperType.getRandom()), 2);
    ...
    public void grow(ServerWorld worldIn, Random rand, BlockPos pos, BlockState state) {
        ...
        worldIn.setBlockState(pos, state.with(AGE, Integer.valueOf(i)).with(VARIANT, PepperType.getRandom()), 2);
    }
}
