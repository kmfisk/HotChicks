...
public class MetalTroughBlock extends ContainerBlock {
    ...
    @Nullable
    public static IInventory getChestInventory(MetalTroughBlock chest, BlockState state, World world, BlockPos pos, boolean override) {
    ...
    public TileEntityMerger.ICallbackWrapper<? extends TroughTileEntity> combine(BlockState state, World world, BlockPos pos, boolean override) {
        ...
        return TileEntityMerger.func_226924_a_(TileEntityRegistry.TROUGH.get(), MetalTroughBlock::getChestMergerType, MetalTroughBlock::getDirectionToAttached, FACING, state, world, pos, bipredicate);
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand
        ...
        if (item == Items.WATER_BUCKET && contains == TroughFillType.NONE) {
            ...
            TroughFillType troughFillType = state.get(MetalTroughBlock.CONTAINS);


            if(state.get(MetalTroughBlock.TYPE) != ChestType.SINGLE && troughFillType == TroughFillType.WATER){
                BlockPos connectedSlot = pos.add(new BlockPos(state.get(MetalTroughBlock.FACING).getDirectionVec()).rotate(state.get(MetalTroughBlock.TYPE) == ChestType.LEFT ? Rotation.CLOCKWISE_90 : Rotation.COUNTERCLOCKWISE_90));

                BlockState connectedState = worldIn.getBlockState(connectedSlot);
                worldIn.setBlockState(connectedSlot, connectedState.with(MetalTroughBlock.CONTAINS, TroughFillType.NONE));
            }
