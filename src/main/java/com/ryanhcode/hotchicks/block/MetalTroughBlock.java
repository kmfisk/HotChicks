...
public class MetalTroughBlock extends ContainerBlock {
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand
        ...
        if (item == Items.WATER_BUCKET && contains == TroughFillType.NONE) {
            ...
            TroughFillType troughFillType = state.getValue(MetalTroughBlock.CONTAINS);


            if (state.getValue(MetalTroughBlock.TYPE) != ChestType.SINGLE && troughFillType == TroughFillType.WATER) {
                BlockPos connectedSlot = pos.offset(new BlockPos(state.getValue(MetalTroughBlock.FACING).getNormal()).rotate(state.getValue(MetalTroughBlock.TYPE) == ChestType.LEFT ? Rotation.CLOCKWISE_90 : Rotation.COUNTERCLOCKWISE_90));

                BlockState connectedState = worldIn.getBlockState(connectedSlot);
                worldIn.setBlockAndUpdate(connectedSlot, connectedState.setValue(MetalTroughBlock.CONTAINS, TroughFillType.NONE));
            }
