...
public class HotVillagerTrades {
    public static void onVillagerTradesEvent(VillagerTradesEvent event) {
        if (event.getType() == VillagerProfession.FARMER) {
            event.getTrades().get(1).add(new VillagerTrades.ItemsForEmeraldsTrade(HotItems.CABBAGE.get(), 1, 4, 16, 1));
            event.getTrades().get(1).add(new VillagerTrades.ItemsForEmeraldsTrade(HotItems.CUCUMBER.get(), 1, 4, 16, 1));
